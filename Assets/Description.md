...
차원 정보가 필요한 곳
플레이어 : 차원 정보를 가지는 필드 추가, 현재 차원을 변경하는 함수 추가
적 : DimensionMember 태그를 쥐어줌


VisionCensor에서 Detection 시 플레이어가 같은 차원에 있는지를 최우선적으로 검사하는 내용을 추가

컨트롤러가 플레이어 오브젝트와 적들의 DimensionMember 컴포넌트들을 가지고 있음
플레이어가 포탈과 상호작용할 때,
포탈이 컨트롤러에게 플레이어가 차원을 바꿨다고 알림
컨트롤러는 플레이어에게 자신의 차원 값을 바꾸도록 한 뒤 바꾼 차원을 받아
그에 맞게 적들에게 자신의 렌더러를 켜고 끄도록 명령